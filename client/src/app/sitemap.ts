...
async function generateDynamicPages(baseUrl: string): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    if (!universities || universities.length === 0) {
      console.log('No universities found, returning empty dynamic pages');
      return dynamicPages;
    }
    ...
    for (const university of universities.slice(0, maxUniversities)) {
      ...
      try {
        ...
        // Continue with next university even if this one fails
        continue;
      }
    }
  } catch (error) {
    console.log('Error generating dynamic pages:', error);
    // Return empty dynamic pages instead of throwing
    return dynamicPages;
  }
